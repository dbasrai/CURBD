...
def configs_for(dataset_tag, have_co9_i0=False):
    ...
    rows = [
        ...
        dict(name='{}_DALE_i0rank_half_flip_noreset'.format(dataset_tag),
             reset_every='none', gamma_init='i0_rank', e_frac=0.5,
             epoch_flip=True, min_e_frac=0.4, flip_margin=0.2,
             why='50% E + flips. State set from data only at the first bin.'),
        dict(name='{}_DALE_i0rank_half_flip_mid'.format(dataset_tag),
             reset_every='mid', gamma_init='i0_rank', e_frac=0.5,
             epoch_flip=True, min_e_frac=0.4, flip_margin=0.2,
             why='50% E + flips. Bout start plus one reset at the middle bin.'),
